...
def get_all_sales():
    try:
        query = "SELECT ID, amount, salesDate, Region.code, Region.name FROM Sales JOIN Region ON Sales.region = Region.code ORDER BY date(salesDate), region"
        with closing(conn.cursor()) as c:
            c.execute(query)
            rows = c.fetchall()
...
def save_all_sales(sales_list):
    ...
    for data in sales_list:
        if data.id == 0:
            with closing(conn.cursor()) as c:
                c.execute(query, (data.amount, data.salesDate, data.region.code))
                conn.commit()

def already_imported(filename):
    try:
        query = "SELECT fileName FROM ImportedFiles WHERE fileName = ?"
...
def add_imported_file(filename):
    query = "INSERT INTO ImportedFiles (fileName) VALUES (?)"
